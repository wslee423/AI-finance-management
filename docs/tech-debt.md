...
| TD-008 | 2026-05-02 | `supabase/add_dividend_soft_delete.sql` 미적용 — dividend 테이블에 `deleted_at` 컬럼 없으면 soft delete API가 에러 발생. Supabase 대시보드 SQL Editor에서 실행 필요 | 🔴 High | 즉시 (배포 전) |
...
| TD-009 | 2026-05-02 | PresetModal type 필드 누락 버그 — insert payload에 `type: p.category` 추가 완료 (고정지출 preset 적용 시 DB.type = null이 되던 버그) |
